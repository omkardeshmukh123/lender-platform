...
export default function Home() {
  ...
  async function fetchLenders() {
    ...
    const { data, error } = await supabase
      .from('lenders')
      .select('*')
      .order('aum_crores', { ascending: false })
  ...
  function applyFilters() {
    ...
    if (loanType) {
      filtered = filtered.filter(l => 
        l.product_types && l.product_types.includes(loanType)
      )
    }
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="bg-blue-600 text-white py-8">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-4xl font-bold mb-2">🏦 Lender Discovery Platform</h1>
          <p className="text-blue-100">Find the right financial partner for your needs</p>
        </div>
      </div>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow p-6">
      ...
      <div className="space-y-2 text-sm text-gray-600 mb-4">
        ...
        {lender.aum_crores && (
          <p><span className="font-medium">AUM:</span> ₹{lender.aum_crores} Cr</p>
        )}
        ...
        {lender.ticket_size_min && lender.ticket_size_max && (
          <p>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1">
            {lender.product_types.slice(0, 3).map(product => (
              <span key={product} className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">
                {product}
              </span>
            ))}
